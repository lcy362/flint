...
Flint 用一个本地优先的 Web 应用解决它：以文件系统为唯一事实源，集中管理 skill、统一打标签与去重、按预设 / 标签**一次性投放**到各 Agent 与项目目录——手动「添加 / 应用预设 / 删除」即时落盘 / 移除，且不被某套自动同步无声改写。
...
2. **跨 Agent 无缝迁移**：任意时刻把预配好的 skill 组合（预设）**一次性应用**到目标 Agent 的 skill 目录，即刻就位、无需手动逐个搬运。
...
- **常驻后台自动同步**：不做常驻进程 / 后台任务来投放或对账；投放只由页面上的显式操作触发（目录级 watcher 已退役，见 SY-04）。
...
| **活跃 Agent（activeAgents）** | 用户标记的 Agent 集合。仅作列表活跃分组 / 排序、主 Agent 判定与诊断扫描的辅助集合；**不再触发或限定任何自动投放**。 |
| **主 Agent（primary）** | 当多个 Agent 解析到**同一个技能目录**时，该目录的分发策略唯一落点（目录只有一份实体，策略只能存一份）。 |
| **Preset（预设）** | 一组命名的 skill 集合 = 显式成员 ∪ 关联标签命中。作为**共享技能包**，通过一次性「应用预设」投放到目标目录；无独立启用开关、不自动同步。 |
| **标签（Tag）** | 给 skill 或项目打的标签：用于过滤浏览、预设关联、项目关联。 |
| **Skill 目录实况（目录）** | agent / 项目当前装了什么技能，**一律读实际目录**为唯一事实；无「应装什么」的期望清单。 |
| **一次性投放（deploy）** | 手动「添加（POST 部署软链 / 副本）」/「应用预设」/「删除」把技能落盘或移除到目标目录的动作；**一次性生效，无按期望集的自动补回**（被删技能不会自动回来）。 |
...
>
> **UI 口径**：`~/.agents/skills` 被生态内绝大多数 Agent 读取，"也会读共享目录"不构成区分特征，因此**不在每个 Agent 上重复「另读」徽标**，而是把「开源生态推荐目录」标在**目录本身**——Agent 列表里那张目录卡换一套强调色突出，主标题即为「开源生态推荐目录」（使用它的 Agent 名退到副标题弱化），标题旁 info 按钮悬停说明「大部分 Agent 都会读这个目录、推荐优先管理；只装给某一个 Agent 请用该 Agent 自己的目录」，且该卡片在**活跃 / 非活跃各自分组内都排第一位**；非活跃时强调色整体减弱、边框换成虚线，既看得出「不在活跃推荐分组」，也仍然与普通卡片区分得开。点进该目录后，详情页的**标题主体同样是「开源生态推荐目录」**（带 info 说明：大部分 Agent 都支持读这个目录，这几个是当前使用的代表），使用它的 Agent 退到标题下方的副行（`~/.agents/skills · Codex / OpenHands / Warp`），「技能目录」一行中该路径也照常标出；活跃状态与切换按**整个目录**处理（见 AA-01）。`~/.config/agents/skills` 等其它共用目录不做特殊标记，与普通目录卡一样正常展示。

### 5.3 活跃 Agent 与一次性投放（AA）

- **AA-01 活跃集合（多选）**：在 Agent 详情页可把 Agent 加入 / 移出活跃集合（`PUT /activeAgents`），集合可含多个成员。活跃只作**展示 / 排序 / 主 Agent 判定**，**不触发任何投放**；删除即不再自动影响目录状态（本就没有自动投放）。因为一个目录只有一套策略，活跃按**目录**判定与切换：同目录任一成员活跃 ⇒ 该目录活跃；详情页的「设为活跃 / 移出活跃」作用于**整个目录**，保证详情页与列表卡片状态口径一致。
- **AA-02 手动投放**：技能进入 / 移出 Agent 目录只经由用户显式操作——**添加**（`POST /agents/:key/skills`，部署软链 / 副本）、**应用预设**（一次性）、**删除**（移除物理产物）。不存在基于结构性变更或活跃集合的自动同步。
- **AA-03 无惰性跟随**：有无活跃与否都不自动改写目录；任何 Agent / 目录的改动都需要在详情页显式发起。
- **AA-04 应用即就位**：在某目录「应用预设」时即刻把展开的成员一次性部署，之后就位；此后不再自动跟随预设或其它配置的变更。
- **AA-05 别名共享**：同目录的多个 Agent 只部署一次（目标折算到主 Agent 去重），一次投放作用于该目录实体、组内一并生效。UI 卡片与详情页同口径：以目录为单位展示「活跃 / 非活跃」，切换也作用于整个目录。同一目录只有一个详情页——标题罗列整组成员（`Codex / OpenHands / Warp`），地址指向别名时前端规范到主 Agent，不把它做成「某个 Agent 自己的页」；头部**不再出现「策略存放于 / 共用目录」这类主别名徽标**（指定主 Agent 见详情页「安装与存放」）。

### 5.4 预设（PR）

- **PR-01 创建 / 编辑 / 删除**：新建预设只填名称；详情页增删显式技能、管理关联标签。
- **PR-02 共享技能包 / 一次性应用**：预设 = 显式 `skills[]` ∪ `tags[]` 命中，作为**可复用的一次性技能包**。**无独立启用开关**；在 Agent / 项目页点「应用预设」时把其展开成员一次性部署进目录，**此后不再自动同步补回**。预设只对显式关联它的 Agent / 目录生效。
- **PR-03 只改定义**：编辑 / 增删预设只更新预设定义（config），**不触达任何既有物理目录**；已部署的目录不会因预设变化而自动增减。
- **PR-04 展示口径统一**：详情页分「当前状态（只读）」与「调整方式（可写）」两组，汇总当前技能与已应用的 Agent。
- **PR-05 关联标签**：`tags[]` 命中的技能在「应用预设」时一并纳入投放；去掉标签后该技能不再属于该预设。预设本身无 on/off 开关，被删除的技能不会因预设仍在而自动回来。

### 5.5 标签（TG）

- **TG-01 打标签**：在技能详情弹窗编辑标签；提供「只看未打标签」过滤便于补全。
- **TG-02 过滤浏览**：按标签 / 来源 / 名称搜索过滤；多选项之间为"或"。
- **TG-03 项目关联**：给项目打标签后，打有相同标签的 skill 会出现在来自该标签的预设 / 技能清单中，可经一次性「应用预设」或手动添加进项目（不自动投放）。
...
- **PJ-01 目录为准**：项目技能列表与状态**完全以 `<project>/.agents/skills` 实际目录为准**；无期望集、无 `explicitOn/explicitOff`、无 on/off 开关。进入 / 移出项目技能只经用户显式「添加 / 删除」。
- **PJ-02 本体目录 `.agents`**：项目内 skill **本体复制**到 `<project>/.agents/skills/`，可提交 git（团队协作必须有文件本体）。
- **PJ-03 其他 Agent 项目目录软链**：各 Agent 的项目级技能目录软链到 `.agents/skills`，实现"一套本体、多 Agent 共享"。实际投放哪些 Agent **以目录结构为事实**（不写 config），通过 `deployedAgents` 反读。
- **PJ-04 一次性投放与清理**：添加动作把技能本体复制到 `.agents/skills` 并写 `INDEX.md` 登记；删除仅移除本工具登记投放的副本（真实目录 / 外部内容永不删）。**无自动同步补回**——被删技能不会自动回来。
- **PJ-05 回写仓库**：`POST /projects/:id/push` 把 `.agents/skills` 中改动的 skill 反向写回指定仓库（仅覆盖仓库中已存在的同名 skill）；UI 在项目详情「回写仓库」。
- **PJ-06 项目技能归集 / 接管**：复用 Agent 侧链路（同一归集弹窗）。**接管落真实副本**而非软链（`.agents` 要提交、跨机器自包含），并登记为项目登记投放的技能。
- **PJ-07 项目技能删除**：仅移除本工具登记投放的副本；用户自带目录 / 外部内容不删。
...
- **SY-04 目录级 watcher（已退役）**：自动投放链取消后它已无事可做——库扫描按请求现算、没有缓存，watcher 唯一的作用就是触发投放；因此实现与设置页开关一并移除，历史配置里的 `watchers` 字段在加载时忽略。
- **SY-05 一次性投放、无自动补回**：技能进入目录只经用户显式「添加 / 应用预设 / 删除」；投放按 diff 驱动（缺则建、失效则重建、不覆盖用户内容）。删除 / 回收只针对 `isManagedLinkTarget` 判定为本工具部署（自有仓库 ∪ 第三方来源）的软链 / 副本，真实目录与外部软链一律不动。
- **SY-06 失败可见**：投放结果 `failed` / `warnings` 直接展示并 toast。

### 5.10 WebUI（UI）

- **UI-01 技术栈**：前端 React + TS + Vite；后端 Node + TS（Express）。
- **UI-02 本地服务**：后端提供 REST API，承担文件扫描、软链 / 复制、配置持久化。
- **UI-03 页面**：
  - **技能库（Library）**：浏览 / 搜索 / 过滤全部 skill；技能详情（SKILL.md 预览 + 标签编辑 + 来源追溯）；底部仓库区（登记 / 编辑 / 删除仓库，归集 / 导入入口）。
  - **智能体（Agents）**：一个实际技能目录一张卡片，展示当前可见技能、活跃态、关联预设、同目录成员；详情页可设活跃、覆盖目录、手动同步、关联预设、**添加 / 删除技能**、按技能覆盖安装方式（无 on/off 开关）。
  - **预设（Presets）**：预设列表与详情（按标签纳入 / 按技能纳入、当前技能、已应用 Agent）。
  - **项目（Projects）**：登记项目、打标签、项目技能列表、**添加 / 删除**、部署到 Agent、归集 / 接管、回写仓库。
  - **诊断（Health）**：6 维度体检 + 就地修复（先确认再执行）。
  - **设置（Settings）**：默认安装方式、自定义 Agent、日志查看 / 下载 / 复制诊断信息。
...
2. 把 `code-review` 预设「应用」到 `trae_cn`、`qoder` 的目录，软链一次性落盘 → 立即可用。
3. 之后改预设只改定义，不追溯这两个目录（被删技能不会自动回来）。
4. 切到 `qwen_code`：在其详情页点「应用预设」即刻就位；其它 Agent 的目录保持现状，不被扰动。
5. 在任意 Agent 中改了某 skill → 回写仓库 → 切回时其它 Agent（若有该软链）也能看到最新版本。

### 6.2 项目级协作

1. 项目 `foo` 打标签 `react`。
2. 把带 `react` 标签的 skill（或对应预设）在各 Agent / 项目页**手动添加 / 应用**——本体复制到 `<foo>/.agents/skills/`，其余 Agent 项目目录软链至 `.agents`，可提交 git 供团队共享。
3. 团队在 `.agents` 中改动 skill → 点「回写仓库」把改动推回仓库本体。
...
```
技能库 / 预设 ──(手动添加 / 应用预设)──▶ 一次性投放 ──▶ 物理目录（软链 / 副本）
     ▲                                              │
     └────────────── 物理为准；无自动补回 ◀───────────┘
```

- **决策（config）**：`repos / foreignSources / customAgents / agents / activeAgents / presets / skillMeta / projects`。
- **技能来源**：仓库、第三方来源、预设，为投放提供候选清单。
- **投放（deploy）**：手动「添加（POST 部署软链 / 副本）」/「应用预设」/「删除」，把技能落盘或移除到目标目录；一次性、幂等，**无自动补回**。
- **目录为准（无期望集）**：agent / 项目当前技能 = 读实际目录，为唯一事实；不维护「应装什么」的期望清单，也不在 config 存 on/off 开关。
- **同步作用域**：无自动同步；`activeAgents` 仅作展示 / 排序 / 主 Agent 判定 / 诊断扫描的辅助。
...
- 首次进入可经「登记仓库 → 归集 / 导入 → 配预设 → 应用预设 / 添加给 Agent」的最小通路把 skill 用起来。
- 任意决策变更（标签 / 预设 / 仓库 / activeAgents）只影响配置与定义，**不追溯既有物理目录**；agent / 项目当前技能以实际目录为准（`diagnose` 可验证目录状态）。
- 物理目录只经用户显式「添加 / 应用预设 / 删除」变更，任何状态变更都不自动改写目录。
- 第三方仓库与 Agent 自带技能的本体不被写入 / 误删；删除只针对本工具部署（`isManagedLinkTarget`）的软链 / 副本。
- 重复执行同一投放，结果幂等、无副作用。
