...
- **含义**：多个 Agent 解析到同一个全局技能目录时（Cline / Warp 共用 `~/.agents/skills`，Amp / Replit 共用 `~/.config/agents/skills`），这组 Agent **共用同一套**预设 / 安装方式 / 显式开关。配置里这套设置**存在其中一个 Agent 名下**（内部称**主 Agent**，它同时是同步落点），**这只是存放位置，不代表策略归它所有**；同目录的其它 Agent 都是平等的真实 Agent，不持有各自生效的策略。存放位置可**显式指定**（`agents[key].primary`，写在被指定的那个 Agent 身上、同目录至多一个），未指定时按「活跃优先，其次名称序」自动判定。
- **为什么**：目录只有一份实体，而策略是按 Agent 存的。同步按 Agent 逐个对账落盘，两套期望集会在同一目录里互相删除（后同步者获胜），所以"同目录各配各的"实质无法生效。
- **佐证**：`core/agents.ts` 的 `primaryOf` / `effectiveAgentKey` / `setPrimary` / `pruneAliasStrategies`（下发给前端 `AgentView.primaryKey` / `primaryExplicit`）；`core/sync.ts` 的 `desiredContext` / `resolveSyncMode` 折算到主 Agent、`syncActive` / `diffSync` 按目录去重；`PUT /agents/:key` 把策略写到主 Agent，并清掉同目录其它 Agent 那份不生效的旧策略（目录类覆盖 `globalDir` / `projectDir` 与 `primary` 指定仍写该 Agent 自己）。
- **约束**：新增任何「按 Agent 存」的分发策略前先问一句"同目录的另一个 Agent 是否也读它"；会读就只能存一份（落在主 Agent 名下），不得为同目录的其它 Agent 保存独立策略。展示层同理：卡片标题罗列使用该目录的全部 Agent（不分主次称呼），策略徽标只展示这一套设置，并用「共用一套策略」点明它们共享；"存在谁名下"只在悬停 / 详情页交代（详情页的「策略存放于」可更换）。同目录其它 Agent 是否"活跃"不影响目录的自动同步（同步目标按目录归并，只要组内有任一活跃成员，该目录就会跟随变更）。
