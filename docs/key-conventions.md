...
- **含义**：多个 Agent 解析到同一个全局技能目录时（Cline / Warp 共用 `~/.agents/skills`，Amp / Replit 共用 `~/.config/agents/skills`），该目录固定选出一个**主 Agent**作为预设 / 安装方式 / 显式开关的唯一落点；同目录的其它 Agent 都是平等的真实 Agent，只是这套策略不由它们决定，也不持有各自生效的策略。主 Agent 可**显式指定**（`agents[key].primary`，写在被指定的那个 Agent 身上、同目录至多一个），未指定时按「活跃优先，其次名称序」自动判定。
...
- **约束**：新增任何「按 Agent 存」的分发策略前先问一句"同目录的另一个 Agent 是否也读它"；会读就必须落到主 Agent，不得为同目录的其它 Agent 保存独立策略。展示层同理：卡片标题罗列使用该目录的全部 Agent（不分主次称呼），策略徽标只展示主 Agent 的生效值并用「策略随 X」交代归属；同目录其它 Agent 是否"活跃"不影响目录的自动同步（同步目标按目录归并，只要组内有任一活跃成员，该目录就会跟随变更）。
